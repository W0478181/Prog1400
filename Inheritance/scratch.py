...
class Rider(Person):
    ...
    def make_trip(self, route, driver):
        print(f"\n{self.first_name} {self.last_name} is requesting a ride to {route.destination}.\n{self.first_name} is available to meet at {route.stop}.")
...
def run():
    ...
    journey1 = [driver1, rider1]

    for i in journey1:
        print(i.details())
        if isinstance(i, Rider):
            i.make_trip(trip, driver1)
